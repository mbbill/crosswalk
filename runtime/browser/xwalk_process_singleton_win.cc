...
#define METRO_REGISTRY_PATH L"Software\\Crosswalk\\Metro"
#define MESSAGE_WINDOW_CLASS L"XWalk_MessageWindow"
#define LAUNCH_MODE_VALUE L"launch_mode"
...
static base::FilePath user_data_dir_;
...
bool ShouldLaunchInWindows8ImmersiveMode(const base::FilePath& user_data_dir) {
  ...
  DWORD reg_value = 0;
  if (reg_key.Create(HKEY_CURRENT_USER, METRO_REGISTRY_PATH,
                     KEY_READ) == ERROR_SUCCESS &&
      reg_key.ReadValueDW(LAUNCH_MODE_VALUE,
...
bool ProcessSingleton::EscapeVirtualization(
  ...
  if (::GetModuleHandle(L"sftldr_wow64.dll") ||
    ...
    for (int tries = 200; tries; --tries) {
      hwnd = ::FindWindowEx(HWND_MESSAGE, NULL, MESSAGE_WINDOW_CLASS,
...
ProcessSingleton::ProcessSingleton(
  ...
  file_util::GetTempDir(&user_data_dir_);
}
...
ProcessSingleton::~ProcessSingleton() {
  ...
  if (window_) {
    ...
    ::UnregisterClass(MESSAGE_WINDOW_CLASS,
